...
type CanvaServiceHandler struct {
	...
	// Drawing history
	history   []*v1.DrawEvent
	historyMu sync.RWMutex

	// Channel for broadcasting messages
	broadcast chan *v1.DrawEvent
...
type client struct {
	id     string
	stream *connect.BidiStream[v1.ClientDrawRequest, v1.ClientDrawResponse]
}
...
func NewCanvaServiceHandler() *CanvaServiceHandler {
	h := &CanvaServiceHandler{
		...
		history:   make([]*v1.DrawEvent, 0, 100),
		broadcast: make(chan *v1.DrawEvent, 100),
...
// Register new client
func (h *CanvaServiceHandler) registerClient(id string, stream *connect.BidiStream[v1.ClientDrawRequest, v1.ClientDrawResponse]) {
	h.clientsMu.Lock()
...
// Send initial history
func (h *CanvaServiceHandler) sendInitialHistory(stream *connect.BidiStream[v1.ClientDrawRequest, v1.ClientDrawResponse]) error {
	h.historyMu.RLock()
	events := make([]*v1.DrawEvent, len(h.history))
	...
	return stream.Send(&v1.ClientDrawResponse{
		Message: &v1.ClientDrawResponse_InitialHistory{
...
func (h *CanvaServiceHandler) addToHistory(event *v1.DrawEvent) {
	h.historyMu.Lock()
	defer h.historyMu.Unlock()
...
func (h *CanvaServiceHandler) broadcastToClients(event *v1.DrawEvent) {
	message := &v1.ClientDrawResponse{
		Message: &v1.ClientDrawResponse_DrawEvent{
			DrawEvent: event,
		},
	}
